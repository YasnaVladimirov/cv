...
- **SETTLED — section rhythm stays at `py-24 md:py-32`.** Design System §5.4 says every top-level section uses it; §3.3 says the spacing *between* major sections is 96px mobile / 128px desktop, and per-section padding produces double that (192px / 256px between adjacent section content). Both readings were put to the human on 2026-08-28 with the measured numbers; they chose to keep what §5.4 builds. §3.3's figure describes the token's purpose, not a target gap. Do not re-litigate.
